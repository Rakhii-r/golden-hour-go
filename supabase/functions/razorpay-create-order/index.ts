...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!installment_id) {
      return json({ error: "Fee plan not set up. Please contact the school." }, 400);
    }

    {
      const { data: t, error: tErr } = await supabase
        ...
        .maybeSingle();
      if (tErr || !t) {
        if (tErr) console.error("term lookup failed", tErr);
        return json({ error: "Invalid installment" }, 404);
      }
    ...
    return json({ error: "Unable to create payment order. Please try again." }, 500);
  }
